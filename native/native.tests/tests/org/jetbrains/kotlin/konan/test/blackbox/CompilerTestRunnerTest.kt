...
abstract class CompilerTestRunnerTestBase : AbstractNativeSimpleTest() {
    ...
    fun testStacktrace() {
        runTest("stacktrace",
            ...
            |##teamcity[testSuiteFinished name='kotlin.test.tests.Ignored']""".trimMargin()
                    val failedOutput = """
            ...
            |##teamcity[testFailed name='bar' message='Bar' details='kotlin.Exception: Bar|n""".trimMargin()

                    val shownInnerException = output.lines().find { it.startsWith("##teamcity[testFailed name='bar'") }
                        ?.contains("Caused by: kotlin.Exception: Baz") ?: false

                    output.contains(ignoredOutput) && output.contains(failedOutput) && shownInnerException
                })
    }
