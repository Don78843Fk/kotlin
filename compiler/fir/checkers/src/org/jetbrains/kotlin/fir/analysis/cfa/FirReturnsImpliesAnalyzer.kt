...
object FirReturnsImpliesAnalyzer : FirControlFlowChecker(MppCheckerKind.Common) {
    ...
    override fun analyze(graph: ControlFlowGraph, reporter: DiagnosticReporter, context: CheckerContext) {
        ...
        val logicSystem = object : LogicSystem(context.session.typeContext) {
            override val variableStorage = object : VariableStorage() {
                // Only care about parameters, which are `val`s.
                override fun RealVariable.isUnstable(): Boolean = false
            }
        }
    ...
    private fun isWrongConditionOnNode(
        ...
        if (operation != null) {
            if (resultExpression is FirLiteralExpression<*>) {
                ...
                val resultVar = logicSystem.variableStorage.getOrCreateIfReal(flow, resultExpression)
                if (resultVar != null) {
